...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const sexo      = interaction.options.getString("sexo"); // ✅ NUEVO

    try {

      const existe = await Ciudadano.findOne({ discord_id: interaction.user.id });
      if (existe) {
      ...
      const roblox = await RobloxVerificado.findOne({ discordId: interaction.user.id });
      if (!roblox) {
        return interaction.reply({
